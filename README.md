...
每次打开先进入“剧本围读助手”首页；首次使用会走两步引导（选择声音来源、选填 DeepSeek Key），之后直接进入上传页。上传页只保留一个上传卡片，音源切换和设置收在顶栏；Qwen 模式下不显示音色库。
...
- 存档续读：合成时逐句把音频写到本地磁盘，围读中自动保存播放位置；换天、关浏览器后可在上传页“继续上次围读”直接接回上次位置
...
- 存档目录可在上传页“设置”里修改，路径每次可自选；换目录后“继续上次围读”会按当前目录列出项目
- 围读中每几秒、暂停或点返回时都会把当前位置写回存档；下次打开网页，点“继续上次围读”，选项目即可从上次位置接续播放
