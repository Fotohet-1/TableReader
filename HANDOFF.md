...
- `{存档目录}/{projectId}/project.json`：剧本/单元/角色，只写一次。
- `{存档目录}/{projectId}/meta.json`：音频记录 + 播放进度。
- 音频：`{存档目录}/{projectId}/audio/{unitId}.wav`。
- 默认目录 `~/Documents/剧本围读存档/`，可在设置改。
- 无数据（0 句）存档自动从列表隐藏；旧版丢数据的需重新上传原剧本。
